...
<div class="page-hero">
    <div class="page-hero-content">
        <h1>{{ $pageContents['tours']->title ?? 'Explore Beautiful Bangladesh' }}</h1>
        <p>{{ $pageContents['tours']->subtitle ?? 'Curated travel packages with premium transport for your perfect getaway.' }}</p>
...
<section style="background:white;">
    <div class="container">
        <div class="section-title">
            <div class="section-label"><i class="fas fa-map-marked-alt"></i> {{ $pageContents['tours']->description ?? 'Destinations' }}</div>
            <h2>{{ $pageContents['tours']->content ?? 'Handpicked Tour Packages' }}</h2>
            <p>{{ $pageContents['tours']->meta['intro_text'] ?? 'Every package includes premium transport, experienced local drivers, and a stress-free journey to Bangladesh\'s finest destinations.' }}</p>
        </div>
        <div class="tours-grid">
            @php
                $tour_items = $pageContents['tours']->meta['tour_items'] ?? null;
                if (is_string($tour_items)) {
                    $tour_items = json_decode($tour_items, true);
                }
            @endphp

            @if($tour_items && count($tour_items) > 0)
                @foreach($tour_items as $item)
                    <div class="tour-card">
                        <div class="tour-img">
                            @if(isset($item['image']) && (strpos($item['image'], 'http') === 0 || strpos($item['image'], 'goride/') === 0))
                                <img src="{{ asset($item['image']) }}" alt="{{ $item['title'] ?? 'Tour' }}">
                            @elseif(isset($item['image']))
                                <img src="{{ asset('storage/page_contents/' . $item['image']) }}" alt="{{ $item['title'] ?? 'Tour' }}">
                            @else
                                <img src="{{ asset('goride/assets/banner/banner_02.jpg') }}" alt="{{ $item['title'] ?? 'Tour' }}">
                            @endif
                            <div class="tour-img-overlay"></div>
                            @if(isset($item['badge']) && $item['badge'])
                                <span class="tour-badge">{{ $item['badge'] }}</span>
                            @endif
                            <div class="tour-price">৳ {{ $item['price'] ?? '0' }}</div>
                        </div>
                        <div class="tour-info">
                            <div class="tour-meta">
                                <span><i class="fas fa-clock"></i> {{ $item['duration'] ?? '' }}</span>
                                <span><i class="fas fa-users"></i> {{ $item['people'] ?? '' }}</span>
                                <span><i class="fas fa-star" style="color:#f59e0b;"></i> {{ $item['rating'] ?? '5.0' }}</span>
                            </div>
                            <h3>{{ $item['title'] ?? '' }}</h3>
